...
impl VM {
  ...
  pub fn interpret<P>(&mut self, source: String) -> InterpretResult where P: parser::Parser {
    ...
    // self.brk += 1;
    self.push_many(&c.clone().into_words());
    ...
    loop {
      ...
      match Opcode::from(instruction) {
        ...
        Opcode::PopClosure => {
          ...
          unsafe {
            Rc::decrement_strong_count(c[2].to_u64() as *const Vec<Word>);
          }
        }
        Opcode::DefineGlobal => {
          // DefineGlobal
        ...
        Opcode::GetLocal => {
          ...
          let v =
            self.stack[
              self.frames[frame_no].frame_pointer + slot..self.frames[frame_no].frame_pointer +
                slot +
                slize
            ].to_vec();
        ...
        Opcode::Call => {
          ...
          let c = Closure::new(f, upvalue_count.to_usize(), upvalue_segment);
        ...
        Opcode::DefineGlobalArray => {
          ...
          for i in (0..num).rev() {
            let v = self.pop_many(slize);
            let static_place = (glob_id as usize) + i * slize + 1;
        ...
        Opcode::Closure => {
          ...
          let uc = self.frames[frame_no].read_byte() as usize;
          ...
          for _ in 0..uc {
            ...
            if is_local {
              ...
              for i in 0..slize {
                upvalue_segment.push(self.frames[frame_no].closure.upvalue_segment[idl + i]);
              }
            }
          }
          let closure = Closure::new(Pointer::Function(p as usize), uc, Rc::new(upvalue_segment));
          self.push_many(&closure.into_words());
        }
        Opcode::GetUpvalue => {
          // getupcal
          ...
          let slize = self.frames[frame_no].read_byte() as usize;
          for i in 0..slize {
            let v = self.frames[frame_no].closure.upvalue_segment[(u_idx as usize) + i];
