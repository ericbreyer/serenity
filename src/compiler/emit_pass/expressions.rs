...
impl EmitWalker {
  ...
  #[instrument(level = "trace", skip_all)]
  pub fn string(&mut self, s: String, _line: usize) -> UValueType {
    let line = 0;
    let mut value = s;
  ...
  #[instrument(level = "trace", skip_all)]
  pub fn deref(&mut self, e: Expression, at: bool, _line: usize) -> UValueType {
    let line = 0;
    // Compile the operand.
  ...
  #[instrument(level = "trace", skip_all)]
  pub fn variable(&mut self, tok: Token, assignment_target: bool, _line: usize) -> UValueType {
    let line = 0;
    ...
    if let Some(i) = arg {
      if assignment_target {
        ...
        self.function_compiler.func.chunk.write_pool_opcode(Opcode::GetLocal, i as u32, line);
      ...
      if assignment_target {
        ...
        self.function_compiler.func.chunk.write_pool_opcode(Opcode::GetGlobal, *i as u32, line);
  ...
  #[instrument(level = "trace", skip_all)]
  pub fn dot(&mut self, e: Expression, tok: Token, at: bool, _line: usize) -> UValueType {
    let line = 0;
    let t = self.visit_expression(e, true);
  ...
  #[instrument(level = "trace", skip_all)]
  pub fn cast(&mut self, value: Expression, cast_type: UValueType, _line: usize) -> UValueType {
    let line = 0;
  ...
  fn resolve_upvalue(&mut self, compiler_level: u32, token: &Token) -> Option<(usize, usize)> {
    ...
    if let Some((upvalue, local)) = self.resolve_upvalue(compiler_level + 1, token) {
      ...
      let Some(_compiler) = Self::compiler_at(compiler_level, &mut self.function_compiler) else {
        return None;
      };
